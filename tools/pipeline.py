...
class Pipeline(object): 
    ...
    def plot_medoid(self): 
        ...
        fig = make_subplots(rows = self.in_ncluster,cols = self.out_ncluster,shared_xaxes=True, shared_yaxes=True)
        ...
        for i in range(self.in_ncluster): 
            for j in range(self.out_ncluster): 
                if idx[i,j] !=  -1:
                    ...
                    fig.add_trace(
                        go.Scatter(x = np.arange(ts_in.shape[0])/self.freq_,y=ts_in, marker_color = 'blue'),
                        row = i+1,
                        col =j+1
                    )
                    ts_out = X_out[idx[i,j]].reshape(-1)
                    fig.add_trace(
                        go.Scatter(x=np.arange(ts_in.shape[0],ts_in.shape[0]+ts_out.shape[0])/self.freq_,y=ts_out,marker_color = 'blue'),
                        row = i+1,
                        col =j+1
                    )
